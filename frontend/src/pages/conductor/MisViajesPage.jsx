...
import { 
  ...
  CheckCircle, XCircle, AlertCircle, Search, ChevronLeft, ChevronRight
} from 'lucide-react';
...
// Componente Calendario Pequeño
const MiniCalendar = () => {
  const [date, setDate] = useState(new Date());
  const daysInMonth = new Date(date.getFullYear(), date.getMonth() + 1, 0).getDate();
  const firstDay = new Date(date.getFullYear(), date.getMonth(), 1).getDay();
  const startDay = firstDay === 0 ? 6 : firstDay - 1; // Ajuste lunes=0
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 animate-in slide-in-from-right duration-500">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-1">
          <button onClick={prevMonth} className="p-1 hover:bg-gray-100 rounded text-slate-500"><ChevronLeft size={16}/></button>
          <button onClick={nextMonth} className="p-1 hover:bg-gray-100 rounded text-slate-500"><ChevronRight size={16}/></button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-7 gap-1 text-center text-xs">
        {[...Array(startDay)].map((_, i) => <div key={`e-${i}`}></div>)}
        {[...Array(daysInMonth)].map((_, i) => {
          const day = i + 1;
          const isToday = new Date().toDateString() === new Date(date.getFullYear(), date.getMonth(), day).toDateString();
          return (
            <div key={day} className={`p-1.5 rounded-full transition-colors cursor-default ${isToday ? 'bg-blue-600 text-white font-bold shadow-md' : 'hover:bg-gray-100 text-gray-700'}`}>
              {day}
            </div>
          );
        })}
...
export default function MisViajesPage() {
  ...
  const loadViajes = async (customFilters = null) => {
    try {
      ...
      const data = await fetchMisViajes(filtersToUse);
  ...
  const getEstadoConfig = (estado) => {
    const configs = {
      ...
      'en_curso': { color: 'bg-amber-50 text-amber-700 border-amber-200', icon: AlertCircle, label: 'En Curso' },
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen font-sans text-slate-800">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 mb-8 overflow-hidden">
            <button onClick={() => setShowFilters(!showFilters)} className="w-full px-6 py-4 flex items-center justify-between text-left hover:bg-gray-50 transition-colors">
              <div className="flex items-center gap-2 text-slate-700 font-medium">
                <Filter size={20} className="text-blue-600" /> Filtrar Historial
              </div>
                      ...
                      return (
                        <div key={viaje.id} className="group bg-white rounded-2xl p-5 border border-gray-200 shadow-sm hover:shadow-lg transition-all duration-200 flex flex-col justify-between">
                          ...
                          <div className="pt-4 border-t border-gray-100 flex justify-between items-center">
                             {(viaje.incidentes) ? (
                                <span className="text-xs font-bold text-red-600 flex items-center gap-1"><AlertCircle size={12}/> Incidente</span>
                             ) : (viaje.observaciones) ? (
                                <span className="text-xs font-medium text-slate-500 flex items-center gap-1"><CheckCircle size={12}/> Observaciones</span>
                             ) : <span></span>}
        ...
        {/* COLUMNA DERECHA: CALENDARIO Y WIDGETS */}
        <div className="hidden lg:block w-80 space-y-6">
          <MiniCalendar />
          
          <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100 text-center">
            <h4 className="font-bold text-blue-900 mb-2">Resumen del Mes</h4>
            <div className="text-4xl font-bold text-blue-600 mb-1">{viajes.length}</div>
            <p className="text-sm text-blue-700">Viajes totales asignados</p>
          </div>
        </div>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-blue-50 rounded-xl p-4 border border-blue-100 text-center">
                   <p className="text-xs text-blue-600 uppercase font-bold mb-1">Salida Real</p>
