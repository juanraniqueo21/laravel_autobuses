...
// Componente Calendario Pequeño (Igual que en conductor pero con colores verdes para asistente)
const MiniCalendar = () => {
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 animate-in slide-in-from-right duration-500">
      ...
      <div className="grid grid-cols-7 gap-1 text-center text-xs">
        ...
        {[...Array(daysInMonth)].map((_, i) => {
          ...
          return (
            <div key={day} className={`p-1.5 rounded-full transition-colors cursor-default ${isToday ? 'bg-green-600 text-white font-bold shadow-md' : 'hover:bg-gray-100 text-gray-700'}`}>
              {day}
            </div>
          );
        })}
    ...
    <div className="p-6 bg-gray-50 min-h-screen font-sans text-slate-800">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1">
          ...
          {/* LISTA DE VIAJES (Igual que antes) */}
          {viajes.length === 0 ? (
        ...
        {/* COLUMNA DERECHA: CALENDARIO Y WIDGETS (NUEVO) */}
        <div className="hidden lg:block w-80 space-y-6">
          <MiniCalendar />
          
          <div className="bg-green-50 p-6 rounded-2xl border border-green-100 text-center">
            <h4 className="font-bold text-green-900 mb-2">Resumen del Mes</h4>
            <div className="text-4xl font-bold text-green-600 mb-1">{viajes.length}</div>
            <p className="text-sm text-green-700">Viajes totales asignados</p>
          </div>
        </div>
