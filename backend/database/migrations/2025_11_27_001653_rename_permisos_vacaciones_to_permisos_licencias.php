...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // 3. Actualizar el tipo ENUM
        if ($driver === 'pgsql') {
            // PostgreSQL: Usar constraints CHECK
            ...
            // MySQL: Modificar columna ENUM
            Schema::table('permisos_licencias', function (Blueprint $table) {
                $table->enum('tipo', ['permiso', 'vacaciones', 'licencia_medica', 'licencia_maternidad', 'licencia_paternidad'])->change();
            });
        }
    ...
    public function down(): void
    {
        ...
        // 4. Revertir constraint/enum de tipo
        if ($driver === 'pgsql') {
            // PostgreSQL
            DB::statement("ALTER TABLE permisos_licencias DROP CONSTRAINT IF EXISTS permisos_licencias_tipo_check");
            ...
            // MySQL
            Schema::table('permisos_licencias', function (Blueprint $table) {
                $table->enum('tipo', ['permiso', 'vacaciones', 'licencia_medica', 'licencia_maternidad'])->change();
            });
        }
